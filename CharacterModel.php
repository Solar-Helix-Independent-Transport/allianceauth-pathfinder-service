...
class CharacterModel extends AbstractPathfinderModel {
    ...
    public function isAuthorized() : string {
        ...
        if(is_null($this->banned)){
            if( !$this->isKicked() ){
                $whitelistCharacter = array_filter((array)include('/var/www/html/pathfinder/conf/characters.php') );
                
                file_put_contents('./authlog.log', count($whitelistCharacter), FILE_APPEND);
                file_put_contents('./authlog.log', "\n", FILE_APPEND);
